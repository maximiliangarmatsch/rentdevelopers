...
class HireForm extends Component {
	...
	sendMail = () =>{
		const { dailyInput, weeklyInput, dayDifference, from, to, email, name } = this.state;
	...
	render(){
		const { dailyInput, weeklyInput, dayDifference, name, email } = this.state;
		const listMembers = this.props.location.state.pickedTeam.map(( member, i ) =>{
			return (
				`${i + 1}. ${member.acf.fullname}`
			)
						...
						<MDBCardBody>
							<MDBRow>
								<MDBCol>
									...
									<MDBInput className="in" value={dailyInput ? String(dailyInput) : ""} max="24"
														onChange={this.onChangeDaily} label="Daily work hours" outline/>

									<MDBInput className="in" value={weeklyInput ? String(weeklyInput) : ""}
														max={dayDifference > 7 ? "7" : String(dayDifference)}
														onChange={this.onChangeWeekly}
														label={`Working days through week(${labelString})`} outline/>


									<MDBInput className="in" value={name} onChange={this.onChangeName} label="Your name"
														outline/>

									<MDBInput type="email" className="in" value={email} onChange={this.onChangeEmail}
														label="Your email" outline/>

									<h4>Total hours: {totalHours}h</h4>
									<hr/>
									<h1>Total Cost: ${totalCost}</h1>
									<div onClick={this.sendMail} className="hireFormSubmit">
										{( totalCost && email && name ) ? <Link to="/order">
												<MDBBtn color="primary">Make an order</MDBBtn>
											</Link> :
											<MDBBtn color="#d84315 deep-orange darken-3">Please fill the calendar and
												inputs</MDBBtn>}
