...
class MemberDetails extends Component {
    ...
    onSubmit = e => {
        ...
        if (this.state.fullnameExist) {
            axios
                .post(
                    `http://ccapp.coder-consulting.com/wp-json/wp/v2/posts/${
                        this.state.id
                    }`,
                    {
                        fields: {
            ...
            axios
                .post(
                    `http://ccapp.coder-consulting.com/wp-json/wp/v2/posts?title=${fullname}&status=publish`,
                    {
                        fields: {
                            fullname,
                            title,
    ...
    fileUploadHandler = event => {
        if (this.state.selectedImage) {
            ...
            axios
                .post(
                    `http://ccapp.coder-consulting.com/wp-json/wp/v2/media`,
                    fd,
                    {
                        headers: {
                            'Content-Type': 'application/json',
    ...
    render() {
        ...
        if (successMess && !imageUploaded && selectedImage) {
            message = (
                <MDBAlert className="mdb-color white-text">
                    <div
                        dangerouslySetInnerHTML={{
                            __html: 'Uploading image...'
                        }}
                    />
                </MDBAlert>
            );
        } else if (
            (successMess && !selectedImage) ||
            (successMess && imageUploaded)
        ) {
            message = (
                <MDBAlert className="mdb-color white-text">
                    Thank you, you info is updated! Go back to User Overview to
                    see changes!
